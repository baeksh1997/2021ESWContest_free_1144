...
public:
    explicit ros_connect(QObject *parent, ros::NodeHandle &nh){
        ...
        init_hunter_pub = nh.advertise<std_msgs::Bool>("/reset_odom_integrator", 100);

        desiredwheel_sub = nh.subscribe("/cmd_vel/des_vel", 1 , &ros_connect::desired_wheel, this);
        racingwheel_sub = nh.subscribe("/cmd_vel/user_input", 1, &ros_connect::racing_wheel, this);
    ...
    Q_INVOKABLE void racing_wheel(const sensor_msgs::Joy::ConstPtr &rwheel){
        ...
        if(rwheel->buttons[6] == 1){
            std_msgs::Bool init_hunter_cmd;
            init_hunter_cmd.data = true;
            init_hunter_pub.publish(init_hunter_cmd);
        }
